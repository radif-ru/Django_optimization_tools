...
class Command(BaseCommand):
    def handle(self, *args, **options):
        # # .filter(category__name='office')
        # # .filter(category__name='modern')
        # # .filter().filter()
        # test_products = Product.objects.filter(
        #     Q(category__name='office') | Q(category__name='modern')
        ...
        action_1__order = When(action_1__cond, then=ACTION_1)  # -> 1
        action_2__order = When(action_2__cond, then=ACTION_2)  # -> 2
        action_expired__order = When(action_expired__cond, then=ACTION_EXPIRED)  # -> 3
